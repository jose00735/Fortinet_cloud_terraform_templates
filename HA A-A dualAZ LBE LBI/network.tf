...
resource "aws_lb_target_group" "target_group_external_" {
  count = length(var.listener_ports)
  name     = format("target-group-external-%s", var.listener_ports[count.index])
...
resource "aws_lb_target_group" "target_group_internal_" {
  count = length(var.listener_ports)
  name     = format("target-group-internal-%s", var.listener_ports[count.index])
  port     = var.listener_ports[count.index]
  protocol = "TCP"
  vpc_id   = aws_vpc.LBE_LBI.id
  target_type = "ip"
...
resource "aws_lb_listener" "lb_listener_external" {
    count = length(var.listener_ports)
    ...
    default_action {
        type = "forward"
        target_group_arn = element(aws_lb_target_group.target_group_external_.*.arn, count.index)
    }
}

resource "aws_lb_listener" "lb_listener_internal" {
    count = length(var.listener_ports)
    ...
    default_action {
        type = "forward"
        target_group_arn = element(aws_lb_target_group.target_group_internal_.*.arn, count.index)
    }
}
...
resource "aws_lb_target_group_attachment" "lb_listener_attachment_fgt1_internal" {
  count = length(var.listener_ports)
  target_group_arn = element(aws_lb_target_group.target_group_internal_.*.arn, count.index)
  target_id        = aws_network_interface.fgt1_public_interface.private_ip
  port             = var.listener_ports[count.index]
...
resource "aws_lb_target_group_attachment" "lb_listener_attachment_fgt2_internal" {
  count = length(var.listener_ports)
  target_group_arn = element(aws_lb_target_group.target_group_internal_.*.arn, count.index)
  target_id        = aws_network_interface.fgt2_public_interface.private_ip
  port             = var.listener_ports[count.index]
